...
Promise<any> | null
{
    ...
    register(userDto
:
    UserCreateDto, invitationId ? : string
):
...
Promise < boolean > {
    const user
:
Pick<UserDto, 'id' | 'status'> = await this.verifyToken(token);
...
Promise < any > {
    ...
    let user
:
UserDto = await this.userRepo.findOne({where: {oauthId}});
...
any
{
    ...
    if (!expiresTime) {
        expiresTime = rememberMe
            ? this.REMEMBER_ME_LIFETIME
